...
**~~Exports are not citable.~~ Fixed in v0.1.0 — see Phase 0 below.** Every
export now carries a provenance sidecar with the version, the full parameter
set and a SHA-256 per volume.
...
## Phase 0 — Make it citable  ✅ DONE (v0.1.0)
...
**Gate — PASSED, measured in the browser.**

| Property | Result |
|---|---|
| same spec twice ⇒ identical digests | PASS |
| one changed parameter ⇒ different digests | PASS |
| restoring that parameter ⇒ original digests return | PASS |

The third row was not in the original plan and is the strongest of the three: it
shows the digest tracks *state* rather than time or GPU nondeterminism.

Incidental confirmation worth recording: adding atrophy changes the T1 and SDF
digests but leaves the **displacement** digest untouched, because atrophy is an
SDF erosion and not a warp. A displacement digest that moved under pure atrophy
